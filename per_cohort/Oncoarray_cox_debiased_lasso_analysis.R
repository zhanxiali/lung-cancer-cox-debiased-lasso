...
#========================================
# Oncoarray - Cox Debiased Lasso åˆ†æž
# åˆ†æž 45 ä¸ª SNP ä¸Žæ€»ç”Ÿå­˜æœŸçš„å…³è” (ä¸åˆ†æžtype) no immuæ·»åŠ  FDR æ ¡æ­£ (Benjamini-Hochberg)
# POs Style to rsID_A   + ä¿®å¤æ›¼å“ˆé¡¿å›¾
#========================================
...
df <- read.table(pick_file("Select the aligned PLINK .raw genotype file (OncoArray)", "GENO_RAW"), header = TRUE, stringsAsFactors = FALSE)
cat("SNP æ•°æ®: ", nrow(df), "ä¾‹,", ncol(df), "åˆ—\n")

# 2. è¯»å–ä¸´åºŠæ•°æ®ï¼ˆå« PCAï¼‰
df2 <- read.csv(pick_file("Select the cleaned clinical CSV (OncoArray)", "CLIN_CSV"), header = TRUE, stringsAsFactors = FALSE)
...
# --- [æ–°å¢ž] æ·»åŠ  FDR æ ¡æ­£ (Benjamini-Hochberg) ---
result$pvalue_FDR <- NA
# åªå¯¹ SNP è¿›è¡Œæ ¡æ­£ï¼Œé¿å…åå˜é‡å¹²æ‰°
if(sum(is_snp) > 0) {
  result$pvalue_FDR[is_snp] <- p.adjust(result$pvalue[is_snp], method = "BH")
  cat("å·²è®¡ç®— FDR (Benjamini-Hochberg)\n")
}
# ==============================================================================
...
# ä¿å­˜æ‰€æœ‰ç»“æžœ
write.csv(result, "Oncoarray_cox_debiased_results_all.csv", row.names = FALSE)
cat("å·²ä¿å­˜: Oncoarray_cox_debiased_results_all.csv\n")

# 3. ä¿å­˜ SNP ç»“æžœ
snp_result <- result[is_snp, ]
snp_result <- snp_result[order(snp_result$pvalue), ]
write.csv(snp_result, "Oncoarray_cox_debiased_results_SNPs.csv", row.names = FALSE)
cat("å·²ä¿å­˜: Oncoarray_cox_debiased_results_SNPs.csv\n")

# 4. ä¿å­˜åå˜é‡ç»“æžœ
covar_result <- result[!is_snp, ]
covar_result <- covar_result[order(covar_result$pvalue), ]
write.csv(covar_result, "Oncoarray_cox_debiased_results_covariates.csv", row.names = FALSE)
cat("å·²ä¿å­˜: Oncoarray_cox_debiased_results_covariates.csv\n")
...
if (nrow(volcano_data) > 0) {
  ...
  p_volcano <- ggplot(volcano_data, aes(x=beta, y=log10p)) +
    ...
    geom_text(data = volcano_data[volcano_data$pvalue < 0.05, ],
    ...
    ylab("-log10(p-value)") +
    ggtitle(paste0("Volcano Plot: ", n_snps, " SNPs - Oncoarray")) +
    theme_bw() +
    theme(plot.title = element_text(hjust = 0.5))
  
  ggsave("Oncoarray_volcano_plot.png", p_volcano, width = 10, height = 8, dpi = 150)
  print(p_volcano)
  cat("å·²ä¿å­˜ç«å±±å›¾: Oncoarray_volcano_plot.png\n")
} else {
...
if (nrow(snp_result) > 0) {
  ...
  p_forest <- ggplot(top_snps, aes(x = HR, y = Variable)) +
    ...
    scale_x_log10() +
    labs(
      title = "Forest Plot: Top 20 SNPs by P-value - Oncoarray",
  ...
  ggsave("Oncoarray_forest_plot_SNPs.png", p_forest, width = 10, height = 8, dpi = 150)
  print(p_forest)
  cat("å·²ä¿å­˜æ£®æž—å›¾: Oncoarray_forest_plot_SNPs.png\n")
}
...
if (nrow(covar_result) > 0) {
  ...
  p_covar <- ggplot(covar_result, aes(x = HR, y = Variable)) +
    ...
    scale_x_log10() +
    labs(
      title = "Forest Plot: Clinical Covariates - Oncoarray",
      subtitle = "Hazard Ratio with 95% CI",
      x = "Hazard Ratio (log scale)",
      y = ""
    ) +
  ...
  ggsave("Oncoarray_forest_plot_covariates.png", p_covar, width = 10, height = 6, dpi = 150)
  print(p_covar)
  cat("å·²ä¿å­˜åå˜é‡æ£®æž—å›¾: Oncoarray_forest_plot_covariates.png\n")
}
...
if (nrow(manhattan_data) > 0) {
  ...
  p_manhattan <- ggplot(manhattan_data, aes(x = CHR, y = log10P)) +
    ...
    labs(x = "Chromosome", 
         ...
         title = "Manhattan Plot - Oncoarray (45 SNPs)") +
  ...
  ggsave("Oncoarray_manhattan_plot.png", p_manhattan, width = 12, height = 6, dpi = 200)
  print(p_manhattan)
  cat("å·²ä¿å­˜æ›¼å“ˆé¡¿å›¾: Oncoarray_manhattan_plot.png\n")
} else {
...
if (nrow(sig_snps_bonf) > 0) {
  ...
  print(sig_snps_bonf[, c("Variable", "CHR", "BP", "beta", "HR", "HR_lower", "HR_upper", "pvalue")])
  
  write.csv(sig_snps_bonf, "Oncoarray_significant_SNPs_bonferroni.csv", row.names = FALSE)
  cat("\nå·²ä¿å­˜: Oncoarray_significant_SNPs_bonferroni.csv\n")
}
...
cat("========================================\n")
cat("\nè¾“å‡ºæ–‡ä»¶:\n")
cat("1. Oncoarray_cox_debiased_results_all.csv - æ‰€æœ‰å˜é‡ç»“æžœ\n")
cat("2. Oncoarray_cox_debiased_results_SNPs.csv - SNP ç»“æžœ (rsID æ ¼å¼)\n")
cat("3. Oncoarray_cox_debiased_results_covariates.csv - åå˜é‡ç»“æžœ\n")
cat("4. Oncoarray_volcano_plot.png - ç«å±±å›¾\n")
cat("5. Oncoarray_forest_plot_SNPs.png - SNP æ£®æž—å›¾ (rsID æ ¼å¼)\n")
cat("6. Oncoarray_forest_plot_covariates.png - åå˜é‡æ£®æž—å›¾\n")
cat("7. Oncoarray_manhattan_plot.png - æ›¼å“ˆé¡¿å›¾ (rsID æ ‡æ³¨)\n")
